...
Usage:
    ...
    python tools/fb_admin.py set-authenticated-claim <uid>
...
def cmd_set_authenticated_claim(token, args):
    uid = args[0]
    url = "%s/v1/projects/%s/accounts:update" % (IDENTITY, PROJECT)
    _request(url, token, "POST", {
        "localId": uid,
        "customAttributes": json.dumps({"role": "authenticated"}),
    })
    print("Firebase custom claim role=authenticated set for %s" % uid)
...
COMMANDS = {
    ...
    "set-authenticated-claim": cmd_set_authenticated_claim,
