...
import com.example.data.supabase.SupabaseUserRepository
...
internal fun authErrorMessage(e: Exception): String {
    ...
    return when {
        ...
        code == "ERROR_NETWORK_REQUEST_FAILED" ||
            ...
            text.contains("GmsNetworkStack", ignoreCase = true) ->
            "تعذر الاتصال بخدمة تسجيل الدخول. تحقق من الإنترنت ثم حاول مرة أخرى"
        text.contains("developer_error", ignoreCase = true) ||
            text.contains("DEVELOPER_ERROR", ignoreCase = true) ->
            "إعدادات تسجيل الدخول عبر Google غير مكتملة، تواصل مع الدعم"
...
class AuthRepository(
    ...
    private val supabaseUsers = SupabaseUserRepository()
    ...
    suspend fun signInWithPassword(email: String, password: String): AuthResult =
        passwordAuth { auth.signInWithEmailAndPassword(email.trim().lowercase(), password).await() }

    suspend fun signUpWithPassword(email: String, password: String): AuthResult =
        passwordAuth { auth.createUserWithEmailAndPassword(email.trim().lowercase(), password).await() }

    private suspend fun passwordAuth(operation: suspend () -> com.google.firebase.auth.AuthResult): AuthResult {
        return try {
            val user = operation().user
            if (user == null) {
                AuthResult.Error("تعذر إنشاء جلسة دخول، حاول مرة أخرى")
            } else {
            ...
            AuthResult.Error(
                when ((e as? com.google.firebase.auth.FirebaseAuthException)?.errorCode) {
                    "ERROR_INVALID_EMAIL" -> "البريد الإلكتروني غير صحيح"
                    "ERROR_EMAIL_ALREADY_IN_USE" -> "هذا البريد مسجل بالفعل، اختر تسجيل الدخول"
                    "ERROR_WEAK_PASSWORD" -> "كلمة المرور ضعيفة، استخدم 6 أحرف على الأقل"
        ...
        val idToken = try {
            ...
            return AuthResult.Error(
                if (e.message?.contains("network", ignoreCase = true) == true ||
                    e.message?.contains("GmsNetworkStack", ignoreCase = true) == true
                ) {
                    "تعذر الاتصال بخدمة Google. تحقق من الإنترنت وخدمات Google Play ثم حاول مرة أخرى"
                } else {
                    "تعذر فتح تسجيل الدخول عبر Google، حاول مرة أخرى"
                }
            )
        }
    ...
    private suspend fun ensureUserDocument(user: FirebaseUser) {
        ...
        // Best-effort identity mirror into Supabase; a failure here must never block sign-in.
        try {
            supabaseUsers.ensureUserDocument(user)
        } catch (_: Exception) {
        }
    }

    suspend fun fetchUserRole(uid: String): String {
        if (BuildConfig.USE_SUPABASE) return supabaseUsers.fetchUserRole(uid)
        return try {
    ...
    suspend fun fetchProviderId(uid: String): String {
        if (BuildConfig.USE_SUPABASE) return supabaseUsers.fetchProviderId(uid)
        return try {
    ...
    suspend fun fetchUserProfile(uid: String): Pair<String, String> {
        if (BuildConfig.USE_SUPABASE) return supabaseUsers.fetchUserProfile(uid)
        return try {
            val snapshot = firestore.collection("users").document(uid).get().await()
    ...
    suspend fun updateUserProfile(uid: String, name: String, address: String) {
        if (BuildConfig.USE_SUPABASE) {
            supabaseUsers.updateUserProfile(uid, name, address)
            return
        }
    ...
    fun signOut() {
        SupabaseConfig.setAuthToken(null)
        auth.signOut()
    }
}
